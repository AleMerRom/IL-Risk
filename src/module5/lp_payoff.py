...
def plot_lp_payoff(
    ...
    *,
    ...
    fig, (ax_value, ax_gap) = plt.subplots(
        2,
        1,
        figsize=(11, 8),
        sharex=True,
    ...
    ax_value.plot([], [], color="gray", linewidth=0.9, linestyle="--", alpha=0.6,
                  label="HODL benchmark (dashed, per position)")

    for ax in (ax_value, ax_gap):
    ...
    ax_value.axhline(100_000, color="dimgray", linestyle=":", linewidth=0.9, alpha=0.5, zorder=4)

    ax_gap.axhline(0, color="dimgray", linestyle=":", linewidth=0.9, alpha=0.65, zorder=4)

    ax_value.set_ylabel("Position value (USD)", fontsize=12)
    ax_gap.set_xlabel("Terminal ETH price $p_T$ (USDC / WETH)", fontsize=12)
    ax_gap.set_ylabel("$V_{LP} - V_{HODL}$ (USD)", fontsize=12)
    ax_value.set_title(
        "LP Value and Relative Payoff vs. Terminal ETH Price — Feeless (Task 5.1.1)\n"
        "The short-straddle-like exposure is the lower panel: $V_{LP}(p_T) - V_{HODL}(p_T)$",
        fontsize=11,
    )
    ax_value.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"${x:,.0f}"))
    ax_gap.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"${x:,.0f}"))
    ax_gap.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{x:,.0f}"))
    ax_value.legend(fontsize=9, loc="upper left", framealpha=0.92)
