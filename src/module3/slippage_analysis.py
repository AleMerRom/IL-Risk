...
from shared.constants import FEE_TIER
from module3.swap_simulator import Direction, simulate_swap
...
def run_simulation_grid(
    *,
    ...
    ``data/results/module_3/simulated_trades.parquet``.  Passing explicit paths makes
    ...
    output_path = Path(output_path) if output_path else DEFAULT_RESULTS_DIR / "simulated_trades.parquet"
...
def summarize_price_impact(
    ...
    *,
    ...
    simulated_trades_path = (
        ...
        else DEFAULT_RESULTS_DIR / "simulated_trades.parquet"
    )
    output_path = (
        Path(output_path)
        if output_path
        else DEFAULT_RESULTS_DIR / "price_impact_summary.parquet"
    )
...
def plot_price_impact_curves(
    ...
    *,
    ...
    output_path = (
        Path(output_path)
        if output_path
        else DEFAULT_FIGURES_DIR / "module3_price_impact_curves.png"
    )
...
def compute_effective_spreads(
    ...
    *,
    ...
    pre_swap_mid_prices_path = (
        ...
        else DEFAULT_RESULTS_DIR / "swap_mid_prices.parquet"
    )
    output_path = (
        Path(output_path)
        if output_path
        else DEFAULT_RESULTS_DIR / "effective_spreads.parquet"
    )
...
def summarize_effective_spreads(
    ...
    *,
    ...
    output_path = (
        Path(output_path)
        if output_path
        else DEFAULT_RESULTS_DIR / "effective_spread_summary.parquet"
    )
...
def plot_effective_spread_comparison(
    ...
    *,
    ...
    output_path = (
        Path(output_path)
        if output_path
        else DEFAULT_FIGURES_DIR / "module3_effective_spread_by_size.png"
    )
...
def _artifact_paths(processed_dir: Path, results_dir: Path, figures_dir: Path) -> dict[str, Path]:
    return {
        ...
        "swap_mid_prices": results_dir / "swap_mid_prices.parquet",
        "simulated_trades": results_dir / "simulated_trades.parquet",
        "price_impact_summary": results_dir / "price_impact_summary.parquet",
        "effective_spreads": results_dir / "effective_spreads.parquet",
        "effective_spread_summary": results_dir / "effective_spread_summary.parquet",
        "price_impact_plot": figures_dir / "module3_price_impact_curves.png",
        "effective_spread_plot": figures_dir / "module3_effective_spread_by_size.png",
    }
...
def check_inputs(
    ...
    paths = _artifact_paths(processed_dir, results_dir, DEFAULT_FIGURES_DIR)
    required = [
...
def simulate(
    ...
    _run_simulation_artifacts(_artifact_paths(processed_dir, results_dir, figures_dir))
...
def effective_spreads(
    ...
    _run_effective_spread_artifacts(
        _artifact_paths(processed_dir, results_dir, figures_dir),
        allow_mid_price_subset=allow_mid_price_subset,
    )
